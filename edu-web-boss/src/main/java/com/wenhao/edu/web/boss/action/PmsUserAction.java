package com.wenhao.edu.web.boss.action;

import com.wenhao.edu.faced.user.common.page.PageBean;
import com.wenhao.edu.faced.user.entity.PmsUser;
import com.wenhao.edu.faced.user.enums.UserStatusEnum;
import com.wenhao.edu.faced.user.enums.UserTypeEnum;
import com.wenhao.edu.faced.user.service.PmsUserFaced;
import com.wenhao.edu.web.boss.base.BaseAction;
...
public class PmsUserAction extends BaseAction {
	...
	@Autowired
	private PmsUserFaced pmsUserFaced;
	...
	public String listPmsUser() {
		try {
			...
			PageBean pageBean = pmsUserFaced.listPage(getPageParam(), paramMap);
	...
	public String viewPmsUserUI() {
		try {
			...
			PmsUser pmsUser = pmsUserFaced.getById(userId);
			if (pmsUser == null) {
	...
	public String addPmsUser() {
		try {
			...
			// 校验用户登录名是否已存在
			PmsUser userNoCheck = pmsUserFaced.findUserByUserNo(userNo);
			if (userNoCheck != null) {
	...
	public String deleteUserStatus() {
		long id = getLong("id");
		pmsUserFaced.deleteUserById(id);
	...
	public String editPmsUserUI() {
		try {
			Long id = getLong("id");
			PmsUser pmsUser = pmsUserFaced.getById(id);
			if (pmsUser == null) {
	...
	public String changeUserStatus() {
		try {
			Long userId = getLong("id");
			PmsUser user = pmsUserFaced.getById(userId);
			if (user == null) {
	...
	public String resetUserPwdUI() {
		PmsUser user = pmsUserFaced.getById(getLong("id"));
		if (user == null) {
	...
	public String resetUserPwd() {
		try {
			...
			pmsUserFaced.updateUserPwd(userId, DigestUtils.sha1Hex(newPwd), false);
	...
	public String userChangeOwnPwd() {
		try {
			...
			// 更新密码
			pmsUserFaced.updateUserPwd(user.getId(), DigestUtils.sha1Hex(newPwd), true);
	...
	public String userViewOwnInfo() {
		try {
			...
			PmsUser user = pmsUserFaced.getById(pmsUser.getId());
			if (user == null) {
